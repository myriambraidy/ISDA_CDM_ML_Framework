...
| fxOption | P1 | fx_option | OptionPayout (vanilla FX option) | Done | `tests/fixtures/fpml/fx_option.xml` |
| fxSingleLegOption | P1 | fx_option (planned) | Same CDM intent as `fxOption` if structure aligns | Planned | Alternate FpML local name — add adapter if corpus differs from `fxOption` |
